...
class Process:
    ...
    @classmethod
    def __handle_nn(cls, hurigana: str, hiragana_dict: Dict[str, str]) ->Tuple[Optional[List[str]], str]:
    ...
    @classmethod
    def __handle_ltu(cls, hurigana: str, hiragana_dict: Dict[str, str]) ->Tuple[Optional[List[str]], str]:
    ...
    @staticmethod
    def __handle_special_letter(hurigana: str, hiragana_dict: Dict[str, str], exception: Set[str], special_char: str = None) ->Tuple[Optional[List[str]], str]:
        #後ろ2文字または1文字が辞書にあれば複数文字のチャンクとして扱う
        chunk_length = 3 if hurigana[1:3] in hiragana_dict else 2# if hurigana[1:2] in hiragana_dict else 0
        if hurigana[1:chunk_length] in hiragana_dict:
            chunk = hurigana[0:chunk_length]
            hurigana = hurigana[chunk_length:]
        #else:
        #    return None, hurigana
        if special_char:
    ...
    #例外処理のない文字
    @staticmethod
    def __handle_pair(hurigana: str, hiragana_dict: Dict[str, str]) ->Tuple[Optional[List[str]], str]:
